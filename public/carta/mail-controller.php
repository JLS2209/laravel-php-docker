...
try {
    ...
    $mail->Username   = 'juan.llontop@unmsm.edu.pe';                     //SMTP username
    $mail->Password   = 'JN72LP15';                               //SMTP password

    //Recipients
    $mail->setFrom('sazon.fuego@example.com', utf8_decode('Sazón & Fuego'));
    $mail->addAddress($_POST['email'], 'Cliente');

    //Content
    $mail->isHTML(true);                                  //Set email format to HTML
    $mail->Subject = utf8_decode('Envío de carta');
    $mail->Body    = utf8_decode('Le hacemos entrega de la carta del restaurante <b>Sazón & Fuego</b>');
    $mail->AltBody = 'Le hacemos entrega de la carta del restaurante Sazón & Fuego';

    //Attachments
    $mail->AddStringAttachment($doc, 'carta-restaurante.pdf', 'base64', 'application/pdf');

    if ($mail->send()) {
        $_SESSION["mensaje"] = 1; // Se envió el mensaje
    } else {
        $_SESSION["mensaje"] = -1; // No se envió el mensaje    
    }

} catch (Exception $e) {
    $_SESSION["mensaje"] = -1; // No se envió el mensaje
    error_log("No se pudo enviar el mensaje. Mailer Error: {$mail->ErrorInfo}");
}
