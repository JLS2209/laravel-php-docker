...
if ($is_fiel == true) {
    $sql = "SELECT id_plato, pl.nombre as nombre, descripcion, imagen, pl.id_categoria, cat.nombre as categoria,
        ...
        WHERE (precio_regular * (100.0 - descuento_fidelidad)/100) >= '$precio'
        ORDER BY (precio_regular * (100.0 - descuento_fidelidad)/100)
        ;";
} else {
    $sql = "SELECT id_plato, pl.nombre as nombre, descripcion, imagen, pl.id_categoria, cat.nombre as categoria,
        (precio_regular * (100.0 - descuento_general)/100) as precio_final_g,
        ...
        FROM tb_plato pl
        INNER JOIN tb_categoria cat ON cat.id_categoria = pl.id_categoria
        WHERE (precio_regular * (100.0 - descuento_general)/100) >= '$precio'
        ORDER BY (precio_regular * (100.0 - descuento_general)/100)
        ;";
}
